...
#if !defined(TENSOR_INCLUDE_SYM_BLOCKED_TENSOR_H)
#define TENSOR_INCLUDE_SYM_BLOCKED_TENSOR_H
...
class SymMOSpace
{
  ...
  private:
    ...
    std::vector<std::vector<size_t>> mos_sym_;
    /// Maps the mo to the pair (sym,rel_mo) where sym is the irrep and rel_mo is
    /// the relative position of a MO within an irrep
