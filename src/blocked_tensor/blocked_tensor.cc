...
void BlockedTensor::add_mo_space(const std::string &name, const std::string &mo_indices, std::vector<size_t> mos,
                                 SpinType spin)
{
    if (name.size() == 0) {
...
void BlockedTensor::add_mo_space(const std::string &name, const std::string &mo_indices,
...
void BlockedTensor::add_composite_mo_space(const std::string &name, const std::string &mo_indices,
        ...
        if (newObject.rank_ > 0) {
            if (newObject.rank_ != this_block.size()) {
                throw std::runtime_error(
                        "Attempting to create the BlockedTensor \"" + name + "\" with nonunique rank.");
            }
        } else {
...
void BlockedTensor::iterate(
        const std::function<void(const std::vector<size_t> &, const std::vector<SpinType> &, double &)> &func)
{
...
void BlockedTensor::citerate(const std::function<void(const std::vector<size_t> &, const std::vector<SpinType> &,
...
void LabeledBlockedTensor::add(const LabeledBlockedTensor &rhs, double alpha, double beta)
{
    ...
    for (std::vector<size_t> &rhs_key : rhs_keys) {
        ...
        if (BlockedTensor::expert_mode()) {
            if (not BT().is_block(lhs_key)) do_add = false;
            if (not rhs.BT().is_block(rhs_key)) do_add = false;
        }
        if (do_add) {
